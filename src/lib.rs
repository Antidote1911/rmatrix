...
fn rand_char(config: Characters) -> String {

    match config {
        config::Characters::Jap => {
            RNG.with(|rng| String::from_utf16(&[(*rng).borrow_mut().gen_range(0xff62..0xff9e)]).unwrap())
        }
        config::Characters::Classic => {
            RNG.with(|rng| String::from_utf16(&[(*rng).borrow_mut().gen_range(33..93)]).unwrap())
        }
        config::Characters::Digits => {
            RNG.with(|rng| String::from_utf16(&[(*rng).borrow_mut().gen_range(48..57)]).unwrap())
        }
    }
        ...
        self.m.iter_mut().for_each(|col| {
            ...
            col.col.iter_mut().for_each(|block| {
                if !in_stream {
                    ...
                    block.val = rand_char(config.characters);
    ...
    pub fn draw(&self, window: &Window, config: &Config) {
        ...
        for j in 1..self.lines {
            for i in 0..self.cols {
                ...
                if config.bold == 2
                    || config.bold == 1
                        && (block.white || block.val.parse::<u32>().unwrap_or_default() % 2 == 0)
                {
                    attrs.set_bold(true);
                }
...
impl Column {
    ...
    fn new_rand_char(&mut self, config: &Config) {
        self.col[0].val = rand_char(config.characters);
